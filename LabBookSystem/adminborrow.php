...
if(isset($_GET['record'])){
	...
	header('Location: adminborrow.php?action=joined');
	exit;		
}
											 ...
											 if ($result->num_rows > 0) {
											  ...
											  while($row = $result->fetch_assoc()){
                                                ...
                                                if($row['isbn']!=null){echo "<td>".$row['isbn']."</td>";}else{echo "<td>"."Collection"."</td>";}
 												echo "<td>".$row['shelf_id']."</td>";
												echo "<td>".$row['borrow_date']."</td>";
												echo '<td><a class="btn btn-small btn-primary" href="adminborrow.php?record='.$recordid.'"><i class="fa fa-reply"></i> return</a></td>';
