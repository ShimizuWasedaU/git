...
public class UserAddAction extends BaseAction{
	...
	public String execute(){
	    ...
	    if(username.length()<=3){
	    	...
	    	if(userService.findUser(username)!=null)error+="<p class='bg-danger'>Username provided is already in use.</p>";	
	    }
	    ...
	    if(!email.matches("[\\w\\.\\-]+@([\\w\\-]+\\.)+[\\w\\-]+")){
	    	...
	    	if(userService.findEmail(email)!=null)error+="<p class='bg-danger'>Email provided is already in use.</p>";
	    }
	    ...
	    if(error.length()<=0){
            ...
            message+="<h2 class='bg-success'>Add new user successfully, please check new user list.</h2>";
            return "adminsuccess";
	    }
	        return "fail";
	}
